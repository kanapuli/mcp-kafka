...
func (k *KafkaHandler) CreateTopic(ctx context.Context, args FuncArgs) (*mcp_golang.ToolResponse, error) {
	...
	if err := kafkaClient.CreateTopic(args.Topic, args.NumPartitions, args.ReplicationFactor); err != nil {
		return nil, err
	}

	return mcp_golang.NewToolResponse(mcp_golang.NewTextContent(fmt.Sprintf("Creating topic %s", args.Topic))), nil
}
